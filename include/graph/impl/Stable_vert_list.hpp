	...
	inline namespace v1 {
		namespace impl {
			...
			struct Stable_vert_list {
				...
				template <class T>
				using Vert_map = persistent_contiguous_map<Vert, T>;
				template <class T>
				auto vert_map(T default_) const {
					return Vert_map<T>(std::move(default_));
				}
				template <class T>
				using Ephemeral_vert_map = ephemeral_contiguous_map<Vert, T>;
				template <class T>
				...
				using Ephemeral_vert_set = ephemeral_contiguous_set<Vert>;
